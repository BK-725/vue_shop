      ...
      <el-card class="box-card">
        ...
        <el-row class="cat_opt">
          <el-col>
            <span>选择商品分类：</span>
            <!-- 选择商品分类区域级联选择框 -->
            <el-cascader size="medium" v-model="selectedCatedKeys" :options="catelist" :props="{ expandTrigger: 'hover', value: 'cat_id', label: 'cat_name', children: 'children', checkStrictly: true }" @change="parentCateChanged" clearable>
            </el-cascader>
          </el-col>
        </el-row>
        <!-- tab 页签区域 -->
...
export default {
  ...
  methods: {
    ...
    // 获取参数的列表数据
    async getParamsData () {
      if (this.selectedCatedKeys.length !== 3) {
        this.selectedCatedKeys = []
        this.manyTableData = []
        this.onlyTableData = []
        return
      }
      // 证明是三级分类
      console.log(this.selectedCatedKeys)
  ...
  // 定义一个计算属性 来获取级联选择框的true 或 false
  computed: {
    isBtnDisabled () {
      // 如果被禁用就返回 true ，否则返回 false
      if (this.selectedCatedKeys.length !== 3) {
        return true
      }
      return false
    ...
    cateId () {
      if (this.selectedCatedKeys.length === 3) {
        return this.selectedCatedKeys[2]
      }
      return null
    },
    // 动态计算属性
